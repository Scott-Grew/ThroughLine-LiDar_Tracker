...
def intersection_over_union_3d(first_box, second_box):
    first_footprint = footprint_polygon(first_box["center_x"], first_box["center_y"], first_box["length"], first_box["width"], first_box["heading"])
    second_footprint = footprint_polygon(second_box["center_x"], second_box["center_y"], second_box["length"], second_box["width"], second_box["heading"])
    footprint_intersection_area = first_footprint.intersection(second_footprint).area
    if footprint_intersection_area <= 0.0:
        return 0.0

    first_z_min, first_z_max = first_box["center_z"] - first_box["height"] / 2.0, first_box["center_z"] + first_box["height"] / 2.0
    second_z_min, second_z_max = second_box["center_z"] - second_box["height"] / 2.0, second_box["center_z"] + second_box["height"] / 2.0
    vertical_overlap = max(0.0, min(first_z_max, second_z_max) - max(first_z_min, second_z_min))
...
def read_predictions(tracks_path, class_name):
    ...
    tracks_table = tracks_table[tracks_table["object_class"] == waymo_type]
...
def main():
    ...
    ground_truth_by_frame = read_ground_truth(arguments.parquet_root, arguments.segment, arguments.class_name)
    predictions_by_frame = read_predictions(arguments.tracks, arguments.class_name)

    accumulator = mm.MOTAccumulator(auto_id=False)
    for frame_timestamp in sorted(set(ground_truth_by_frame) | set(predictions_by_frame)):
        ground_truth_boxes = ground_truth_by_frame.get(frame_timestamp, [])
        prediction_boxes = predictions_by_frame.get(frame_timestamp, [])

        ground_truth_ids = [box["object_id"] for box in ground_truth_boxes]
        prediction_ids = [box["track_id"] for box in prediction_boxes]

        distance_matrix = np.full((len(ground_truth_boxes), len(prediction_boxes)), np.nan)
        for ground_truth_index, ground_truth_box in enumerate(ground_truth_boxes):
            for prediction_index, prediction_box in enumerate(prediction_boxes):
                iou = intersection_over_union_3d(ground_truth_box, prediction_box)
                if iou >= arguments.iou_threshold:
                    distance_matrix[ground_truth_index, prediction_index] = 1.0 - iou

        accumulator.update(ground_truth_ids, prediction_ids, distance_matrix, frameid=frame_timestamp)

    metrics_host = mm.metrics.create()
    summary = metrics_host.compute(
        accumulator,
        metrics=["num_frames", "mota", "motp", "idf1", "num_switches", "num_misses", "num_false_positives", "num_objects"],
        name=f"{arguments.segment}/{arguments.class_name}",
    )

    print(f"scored with motmetrics {mm.__version__} and shapely (third-party, not this project's own code)")
