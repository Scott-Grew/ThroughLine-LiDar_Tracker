...
void export_tracks(const std::string& path, const SegmentLog& segment,
  ...
  for (std::size_t frame_index = 0;
       frame_index < confirmed_tracks_per_frame.size();
       ++frame_index) {
    const Frame& frame = segment.frames[frame_index];
    const Eigen::Matrix4d vehicle_from_world =
        frame.vehicle_to_world.inverse();
    const double heading_offset = std::atan2(
        frame.vehicle_to_world(1, 0), frame.vehicle_to_world(0, 0));

    for (const Track& track :
         confirmed_tracks_per_frame[frame_index]) {
      const Eigen::Vector4d world_position(track.state.mean(0),
                                           track.state.mean(1),
      ...
      stream << frame.capture_time_micros << ',' << track.track_id
             ...
             << ',' << vehicle_position(2) << ','
             << track.state.length << ',' << track.state.width << ','
             << track.state.height << ',' << vehicle_yaw << '\n';
    }
  }
}
