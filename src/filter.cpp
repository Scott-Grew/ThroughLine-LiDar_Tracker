...
// wrong each of those might be. Nothing outside this file changes those numbers. The tracker calls
// predict when time passes and update when a new box arrives; the assignment code calls
// mahalanobis_squared to ask whether a box could belong to this object; the viewer draws the
// uncertainty this file produces. Speed and turn rate are never measured by anything - they exist
// only because this file infers them from how the box moves between frames.

namespace {

// How unsure we are about speed and turning the very first time we see an object. Both are guesses
// of zero, so the uncertainty is set wide enough to cover a parked car through a highway car,
// which lets the first few frames correct them freely instead of fighting a confident zero.
constexpr double kInitialSpeedSigma = 10.0;
constexpr double kInitialYawRateSigma = 1.0;

// Below this turn rate an object counts as going straight. The turning formula divides by the turn
// rate, so without this cutoff every straight-driving car would divide by zero and poison its
// track with invalid numbers. Straight driving is the common case, so this branch runs constantly.
constexpr double kStraightLineYawRate = 1e-4;

// How much of a newly measured box size to believe each frame. A car's length does not change, so
// size is smoothed separately rather than being estimated like position and speed.
constexpr double kSizeSmoothing = 0.3;

// Says which of the five numbers the sensor actually sees: position and facing, never speed or
// turn rate. Everything that compares a prediction to a measurement goes through this.
Eigen::Matrix<double, 3, 5> measurement_jacobian() {
  Eigen::Matrix<double, 3, 5> matrix = Eigen::Matrix<double, 3, 5>::Zero();
  matrix(0, 0) = 1.0;
  matrix(1, 1) = 1.0;
  matrix(2, 2) = 1.0;
  return matrix;
...
// How much the box itself jitters frame to frame. These values are measured from the data by the
// staging script, not invented here. They decide how much the filter trusts each new box.
Eigen::Matrix3d measurement_noise(const FilterNoise& noise) {
  Eigen::Matrix3d matrix = Eigen::Matrix3d::Zero();
  matrix(0, 0) = noise.sigma_measurement_position * noise.sigma_measurement_position;
  matrix(1, 1) = noise.sigma_measurement_position * noise.sigma_measurement_position;
  matrix(2, 2) = noise.sigma_measurement_yaw * noise.sigma_measurement_yaw;
  return matrix;
}

}

// Folds an angle back into the range minus half a turn to plus half a turn. Facing angles wrap
// around, so two cars pointing almost the same way can differ by nearly a full turn on paper. Skip
...
TrackState initial_state(const Box& box, const FilterNoise& noise) {
  ...
  state.covariance(0, 0) = noise.sigma_measurement_position * noise.sigma_measurement_position;
  state.covariance(1, 1) = noise.sigma_measurement_position * noise.sigma_measurement_position;
  state.covariance(2, 2) = noise.sigma_measurement_yaw * noise.sigma_measurement_yaw;
  state.covariance(3, 3) = kInitialSpeedSigma * kInitialSpeedSigma;
  state.covariance(4, 4) = kInitialYawRateSigma * kInitialYawRateSigma;
...
// Moves an object forward in time without looking at any new data, and widens its uncertainty
...
void predict(TrackState& state, double dt_seconds, const FilterNoise& noise) {
  ...
  if (std::abs(yaw_rate) < kStraightLineYawRate) {
    ...
    jacobian(0, 4) = radius * dt_seconds * std::cos(yaw_next) - radius * (std::sin(yaw_next) - std::sin(yaw)) / yaw_rate;
    jacobian(1, 2) = radius * (std::sin(yaw_next) - std::sin(yaw));
    jacobian(1, 3) = (std::cos(yaw) - std::cos(yaw_next)) / yaw_rate;
    jacobian(1, 4) = radius * dt_seconds * std::sin(yaw_next) - radius * (std::cos(yaw) - std::cos(yaw_next)) / yaw_rate;
...
// The plain disagreement between where a box is and where this object was predicted to be. The
// facing part is wrapped, because the shortest way round is the only meaningful answer.
...
// How much disagreement would be normal, given both how unsure the prediction is and how much the
// sensor jitters. Without this there is no way to tell a small error from a large one.
Eigen::Matrix3d innovation_covariance(const TrackState& state, const FilterNoise& noise) {
  const Eigen::Matrix<double, 3, 5> observation = measurement_jacobian();
  return observation * state.covariance * observation.transpose() + measurement_noise(noise);
}

// Turns a disagreement in metres into a disagreement in units of how surprised we should be. Two
// metres is enormous for an object we have watched for a second and nothing at all for one we just
// found. The assignment code uses this number to decide which box belongs to which object, and to
// refuse a pairing that is too far-fetched, so this is the gatekeeper against identity swaps.
double mahalanobis_squared(const TrackState& state, const Box& measurement, const FilterNoise& noise) {
  ...
  const Eigen::Matrix3d covariance = innovation_covariance(state, noise);
  return difference.transpose() * covariance.ldlt().solve(difference);
}

// Folds a new box into the object, pulling the five numbers toward what was just seen. How far they
// move is decided by which is currently more trustworthy, the prediction or the sensor - nothing is
// hand-tuned here. Because position and speed have become linked by watching the object move,
// correcting the position also corrects the speed, which is how a quantity the sensor never
// measures gets learned at all. The long covariance line is the numerically safe form of the
// update; the short textbook version drifts until the uncertainty stops being valid and the
// gatekeeper above starts making nonsense decisions. Box size is not estimated, only smoothed.
void update(TrackState& state, const Box& measurement, const FilterNoise& noise) {
  const Eigen::Matrix<double, 3, 5> observation = measurement_jacobian();
  const Eigen::Vector3d difference = innovation(state, measurement);
  const Eigen::Matrix3d covariance = innovation_covariance(state, noise);
  const Eigen::Matrix<double, 5, 3> gain = state.covariance * observation.transpose() * covariance.inverse();

  state.mean += gain * difference;
  state.mean(2) = wrap_angle(state.mean(2));

  const Eigen::Matrix<double, 5, 5> factor = Eigen::Matrix<double, 5, 5>::Identity() - gain * observation;
  state.covariance = factor * state.covariance * factor.transpose() + gain * measurement_noise(noise) * gain.transpose();
