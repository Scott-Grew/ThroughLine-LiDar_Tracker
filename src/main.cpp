...
void print_summary(const TimingStats& timing) {
  std::cout << "step_p50_ms " << timing.percentile(0.5)
            ...
            << " overruns " << timing.overruns << " frames "
            << timing.step_milliseconds.size() << "\n";
}
...
int run(int argument_count, char** arguments) {
  ...
  app.add_option("--segment", segment_path,
                 "staged .trklog to replay")
      ->required();
  app.add_option("--assign", assignment_name, "hungarian or greedy")
      ->check(CLI::IsMember({"hungarian", "greedy"}));
  app.add_option("--dropout",
                 settings.perturbation.dropout_probability,
                 "probability a detection is dropped");
  app.add_option("--noise",
                 settings.perturbation.position_noise_metres,
  ...
  app.add_option("--sigma-yaw",
                 settings.tracker.noise.sigma_measurement_yaw,
  ...
  app.add_option("--export", export_path,
                 "write confirmed tracks as CSV");
  ...
  if (app.count("--sigma-yaw") == 0)
    settings.tracker.noise.sigma_measurement_yaw =
        segment.measured_yaw_sigma;
  Replay replay(segment, settings);
  replay.run();
  print_summary(replay.timing());

  if (!export_path.empty())
    export_tracks(export_path, segment,
                  replay.confirmed_tracks_per_frame());
  if (!record_path.empty()) {
    const ConstantTurnRatePredictor predictor(settings.tracker.noise);
    save_replay_recording(
        record_path, segment, replay.confirmed_tracks_per_frame(),
        predictor, kPredictionHorizonSeconds, kPredictionStepSeconds);
  }
  return 0;
}
