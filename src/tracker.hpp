...
struct TrackerSettings {
  ...
  // Max squared Mahalanobis distance (x, y, yaw gap in units of filter uncertainty) for a match.
  // Chi-squared, 3 degrees of freedom, 0.99 quantile: a true match is refused 1 percent of the time.
  double gate_chi_squared = 11.34;
  // Track slots reserved up front so the list never reallocates mid-run. Chosen.
...
class Tracker {
 public:
  ...
  std::vector<Track> 
    confirmed_tracks_at(std::int64_t query_time_micros) const;
 private:
