...
def read_components(parquet_root, segment, laser):
    ...
    calibration_table = pq.read_table(
        f"{parquet_root}/lidar_calibration/{segment}.parquet"
    )
    pose_table = pq.read_table(
        f"{parquet_root}/vehicle_pose/{segment}.parquet"
    )
        ...
        frames.append(
            {
                ...
                "vehicle_to_world": np.array(
                    pose_by_frame[frame_timestamp], dtype=np.float64
                ).reshape(4, 4),
...
def range_image_to_points(
    ...
    range_image = np.array(
        range_image_values, dtype=np.float64
    ...
    inclination_grid, azimuth_grid = np.meshgrid(
        inclinations, azimuths, indexing="ij"
    )
    ...
    sensor_x = (
        ranges * np.cos(valid_inclinations) * np.cos(valid_azimuths)
    )
    sensor_y = (
        ranges * np.cos(valid_inclinations) * np.sin(valid_azimuths)
    )
    sensor_z = ranges * np.sin(valid_inclinations)

    sensor_points = np.stack(
        [sensor_x, sensor_y, sensor_z, np.ones_like(sensor_x)], axis=1
    )
...
def count_points_in_box(points, box):
    ...
    inside_footprint = (np.abs(local_x) <= box["length"] / 2.0) & (
        np.abs(local_y) <= box["width"] / 2.0
    )
    inside_height = (
        np.abs(points[:, 2] - box["center_z"]) <= box["height"] / 2.0
    )
...
def measure_box_jitter(boxes_by_frame):
    ...
    for frame_index, boxes in enumerate(boxes_by_frame):
        for box in boxes:
            positions_by_object.setdefault(
                box["object_id"], []
            ).append(
                (
                    frame_index,
    ...
    for trajectory in positions_by_object.values():
        for index in range(1, len(trajectory) - 1):
            ...
            current_frame, current_x, current_y, current_heading = (
                trajectory[index]
            )
            next_frame, next_x, next_y, next_heading = trajectory[
                index + 1
            ]
            if (
            ...
            position_residuals.append(
                next_y - (2.0 * current_y - previous_y)
            )
            heading_residuals.append(
                wrap_angle(
                    next_heading
                    - (2.0 * current_heading - previous_heading)
                )
            )

    position_sigma = (
        float(np.std(position_residuals))
        if position_residuals
        else 0.0
    )
    heading_sigma = (
        float(np.std(heading_residuals)) if heading_residuals else 0.0
    )
    print(
        f"measured sigma_measurement_position: {position_sigma:.6f} m"
    )
...
def write_log(
    ...
    with open(path, "wb") as stream:
        ...
        stream.write(
            struct.pack("<2d", position_sigma, heading_sigma)
        )
        for frame in frames:
            stream.write(
                struct.pack("<q", frame["capture_time_micros"])
            )
            stream.write(
                struct.pack(
                    "<16d",
            ...
            for box in ground_truth:
                ...
                stream.write(
                    struct.pack("<i", box["num_lidar_points_in_box"])
                )
...
def main():
    ...
    frames = read_components(
        arguments.parquet_root, arguments.segment, arguments.laser
    )

    if frames:
        first_frame = frames[0]
        staged_points_in_boxes = sum(
            count_points_in_box(first_frame["points"], box)
            for box in first_frame["ground_truth"]
        )
        reported_points_in_boxes = sum(
            box["num_lidar_points_in_box"]
            for box in first_frame["ground_truth"]
        )
        ratio = (
            staged_points_in_boxes / reported_points_in_boxes
            if reported_points_in_boxes
            else 0.0
        )
        print(
            f"staged/reported point ratio for first frame: {ratio:.4f}"
        )
        if not 0.5 <= ratio <= 1.5:
            print(
                "WARNING: point ratio far from 1 - the azimuth convention is likely wrong"
            )

    position_sigma, heading_sigma = measure_box_jitter(
