...
// A shared way for the tests to get a segment to run against, without
// any of them staging real Waymo data or duplicating the same
// synthetic scene by hand. Every object moves on a constant turn rate
// path with its own speed and turn rate, seeded so the same seed
// always builds the same segment, which is what lets the determinism
// and allocation tests compare two runs against a segment they know
// is identical.
inline SegmentLog make_synthetic_segment(int frame_count,
                                         int object_count,
                                         std::uint64_t seed) {
  ...
  for (int object_index = 0; object_index < object_count;
    ...
    objects[object_index] =
        ObjectState{static_cast<double>(object_index) * 50.0, 0.0,
                    heading_draw(generator), speed_draw(generator),
                    yaw_rate_draw(generator)};
  }
  ...
  for (int frame_index = 0; frame_index < frame_count;
    ...
    frame.capture_time_micros =
        static_cast<std::int64_t>(frame_index) * 100000;
    ...
    for (int object_index = 0; object_index < object_count;
      ...
      if (std::abs(object.yaw_rate) < 1e-4) {
        object.x +=
            object.speed * std::cos(object.yaw) * kStepSeconds;
        object.y +=
            object.speed * std::sin(object.yaw) * kStepSeconds;
      } else {
        const double yaw_next =
            object.yaw + object.yaw_rate * kStepSeconds;
        const double radius = object.speed / object.yaw_rate;
        object.x +=
            radius * (std::sin(yaw_next) - std::sin(object.yaw));
        object.y +=
            radius * (std::cos(object.yaw) - std::cos(yaw_next));
        object.yaw = yaw_next;
      }
    }
  }
