...
// This file checks the Hungarian method against the only thing that
// can be trusted more than an algorithm: trying every possibility by
// hand. A small enough cost matrix can be solved by brute force in a
// few hundred thousand steps, so 200 random gated matrices are solved
// twice, once by the algorithm actually used in the tracker and once
// by exhaustive search, and the two totals are required to agree.
// Matching more pairs always beats a lower-cost solution with fewer
// pairs - that is what the tracker actually wants, since an unmatched
// track becomes a miss - so both solvers score a candidate solution
// the same way: pair count first, total cost second.
...
double best_brute_force_objective(const Eigen::MatrixXd& cost,
  ...
  for (int column = 0; column < cost.cols(); ++column) {
    ...
    const double candidate = best_brute_force_objective(
  ...
  for (int trial = 0; trial < 200; ++trial) {
    ...
    for (int row = 0; row < row_count; ++row)
      for (int column = 0; column < column_count; ++column)
        cost(row, column) =
            unreachable_draw(generator) < 0.2
    ...
    const double hungarian_objective =
        objective(static_cast<int>(assignment.pairs.size()),
                  hungarian_total_cost);

    std::vector<bool> column_used(column_count, false);
    const double brute_force_result = best_brute_force_objective(
        cost, kGate, 0, column_used, 0, 0.0);
